...
class Master:
    ...
    def load_configuration(self):
        config_dir = os.path.dirname(os.path.abspath(SRC.UTILS.CONFIG.__file__))
        config_result = ConfigManager(config_dir=config_dir, config_name='unencoded_example.config_encoded.json').open_config()
        if isinstance(config_result, tuple) and config_result[0] and config_result[1]:
    ...
    def set_security(self):
        try:
            ...
            self.SE = SE(
                key_location=SE.base64_decode_string(encoded_string=key_location),
                secret_key=SE.base64_decode_string(
                    encoded_string=self.CF.return_config_secrets(secrets_fernet_key=True)),
                salt_key=SE.base64_decode_string(encoded_string=self.CF.return_config_secrets(secrets_salt_key=True)),
            )
    ...
    def set_database_manager(self):
        try:
            ...
            self.DB = DB(
                user=self.SE.base64_decode_string(encoded_string=self.CF.return_config_database(database_user=True)),
                password=self.SE.base64_decode_string(
                    encoded_string=self.CF.return_config_database(database_password=True)),
                host=self.SE.base64_decode_string(encoded_string=self.CF.return_config_database(database_host=True)),
                port=self.SE.base64_decode_string(encoded_string=self.CF.return_config_database(database_port=True)),
                database=self.SE.base64_decode_string(
                    encoded_string=self.CF.return_config_database(database_database=True)),
    ...
    def set_ai(self):
        try:
            ...
            self.AI = AI(
                OPEN_ROUTER_KEY=self.SE.base64_decode_string(encoded_string=self.CF.return_config_ai_keys(ai_keys_OpenRouter_Key=True)),
                OLLAMA_KEY=self.SE.base64_decode_string(encoded_string=self.CF.return_config_ai_keys(ai_keys_Ollama_Key=True)),
                OPEN_AI_KEY=self.SE.base64_decode_string(encoded_string=self.CF.return_config_ai_keys(ai_keys_OpenAi_key=True))
